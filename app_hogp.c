...
const BLE_CHAR AttCharList[] = {
  ...
  {TYPE_CHAR, 0x11, {ATT_CHAR_PROP_RD | ATT_CHAR_PROP_W | ATT_CHAR_PROP_W_NORSP | ATT_CHAR_PROP_NTF, 0x12, 0, 0 },1}, // Characteristic
...
void att_server_rdByGrType( uint8_t pdu_type, uint8_t attOpcode, uint16_t st_hd, uint16_t end_hd, uint16_t att_type )
{
  ...
  else if ((att_type == GATT_PRIMARY_SERVICE_UUID) && (st_hd <= 0x11))
  {
